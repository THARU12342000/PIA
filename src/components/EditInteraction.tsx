...
const EditInteraction = () => {
  ...
  const fetchInteraction = async () => {
    try {
      ...
      if (data.interactionDate?.startDateTime) {
        data.interactionDate.startDateTime = new Date(data.interactionDate.startDateTime).toISOString().slice(0, 16);
      }
      if (data.interactionDate?.endDateTime) {
        data.interactionDate.endDateTime = new Date(data.interactionDate.endDateTime).toISOString().slice(0, 16);
      }
      setFormData(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await api.patch(`/partyInteraction/${id}`, formData);
      toast.success('Interaction updated successfully');
      navigate(`/interactions/${id}`);
    } catch (error: any) {
      toast.error(error.response?.data?.error || 'Failed to update interaction');
    } finally {
      setLoading(false);
    }
  };
  ...
  const removeInteractionItem = (index: number) => {
    setFormData((prev: any) => ({
      ...prev,
      interactionItem: prev.interactionItem.filter((_: any, i: number) => i !== index)
    }));
  };
  ...
  const updateInteractionItem = (index: number, field: string, value: string) => {
    setFormData((prev: any) => ({
      ...prev,
      interactionItem: prev.interactionItem.map((item: any, i: number) => 
  ...
  const removeNote = (index: number) => {
    setFormData((prev: any) => ({
      ...prev,
      note: prev.note.filter((_: any, i: number) => i !== index)
    }));
  };
  ...
  const updateNote = (index: number, field: string, value: string) => {
    setFormData((prev: any) => ({
      ...prev,
      note: prev.note.map((note: any, i: number) => 
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          ...
          {formData.interactionItem.length === 0 ? (
            ...
            <div className="space-y-4">
              {formData.interactionItem.map((item: any, index: number) => (
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          ...
          {formData.note.length === 0 ? (
            ...
            <div className="space-y-4">
              {formData.note.map((note: any, index: number) => (
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
